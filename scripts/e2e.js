...
(async () => {
  ...
  try {
    ...
    const analysisBlock = await page.$x("//h3[contains(., 'Analysis')]/following::p[1]");
    if (!analysisBlock.length) failures.push('No Analysis paragraph');

    // Sentiment present
    const sentimentLabel = await page.$x("//*[contains(text(),'Sentiment:')] ");
    if (!sentimentLabel.length) failures.push('No Sentiment block');
